...
- `RecordingTimer.kt` — `VideoRecordingPill`: red capsule + pulsing white dot + `M:SS` caption, ticked every 500ms. Rendered BottomCenter-aligned inside the preview slab Box during a live video recording so the pill overlays the preview without pushing the zoom chips / shutter row / queue down.
...
- `LocalSendSheet.kt` — `ModalBottomSheet` that opens with the selected bundles. Three internal states: `Discovering` (timed-out flag for a 12-second "no peers found" fallback with Try again / Close), `Sending(peerAlias, bundleCount, progress)`, `Done(bundleCount, result)`. The sheet's `closeAndDismiss` calls `onDismiss()` synchronously and offloads the discovery / send teardown to `container.appScope` so the modal scrim stops blocking touches the moment the user taps Close — the scope captured by the sheet's `rememberCoroutineScope` is cancelled the instant the parent removes the sheet from composition, so anything that needs to *survive* dismissal (in particular `stopDiscovery`'s socket close) has to run on a process-scoped scope.
